...
type ServerInterface interface {
	...
	// Find-or-create the canonical product for a search selection
	// (POST /products/resolve)
	ResolveProduct(w http.ResponseWriter, r *http.Request)
	// Fetch a product (read-through with per-type staleness)
	// (GET /products/{productId})
	GetProduct(w http.ResponseWriter, r *http.Request, productId openapi_types.UUID)
	// Score unowned games against a library summary (user-agnostic)
	// (POST /recommendations:score)
	ScoreRecommendations(w http.ResponseWriter, r *http.Request)
	// Discovery search (game metadata via IGDB, hardware via PriceCharting)
	// (GET /search)
...
func HandlerWithOptions(si ServerInterface, options StdHTTPServerOptions) http.Handler {
	...
	m.HandleFunc("POST "+options.BaseURL+"/recommendations:score", wrapper.ScoreRecommendations)
	m.HandleFunc("GET "+options.BaseURL+"/search", wrapper.SearchCatalog)
