...
// auth and every API route inside it. Readiness checks Postgres only:
// Valkey is a soft dependency (the dashboard cache fails open
// per-request).
func NewRouter(h *Handlers, v *jwtauth.Validator, logger *slog.Logger, ready func(context.Context) error) http.Handler {
	...
	return httpkit.NewRouter("collection", authed, apiMux, logger, ready)
}
