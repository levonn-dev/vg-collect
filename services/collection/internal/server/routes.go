...
// NewRouter builds the API handler behind jwtauth.Middleware and specval's
// request validation, then httpkit.NewRouter wires Recover -> otelhttp span ->
// RequestLogger -> mux, with /healthz and /readyz outside JWT auth. Readiness
// checks Postgres only; Valkey is a soft dependency (cache fails open).
//
// specval sits AFTER jwtauth (jwtauth's 401 keeps precedence) and wraps only
// the generated API handler, so a 404/405 the spec has nothing to say about still passes through untouched.
