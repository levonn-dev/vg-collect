...
// LibraryGame is one deduplicated owned game for recommendation scoring: the
// best rating across copies, dropped only when every copy is dropped.
...
func (s *Store) LibrarySummary(ctx context.Context, userID uuid.UUID) ([]LibraryGame, error) {
	...
	return pgkit.ScanAll(rows, []LibraryGame{}, func(r pgx.Rows) (LibraryGame, error) {
		var g LibraryGame
...
func (s *Store) CoverURLs(ctx context.Context, userID uuid.UUID, f Filters, limit int) ([]string, error) {
	...
	return pgkit.ScanAll(rows, []string{}, func(r pgx.Rows) (string, error) {
		var u string
...
// DashboardCounts aggregates the user's collection, narrowed by the same
// filter matrix as ListEntries. The five reads run inside one repeatable-read,
// read-only transaction so a concurrent write can't make Total disagree with the grouped sums.
func (s *Store) DashboardCounts(ctx context.Context, userID uuid.UUID, f Filters) (DashboardCounts, error) {
	...
	txErr := pgx.BeginTxFunc(ctx, s.pool,
		...
		func(tx pgx.Tx) error {
			if err := tx.QueryRow(ctx,
				...
				return fmt.Errorf("store: dashboard total: %w", err)
			}
			...
			if err := groupInto(
				...
				return fmt.Errorf("store: dashboard status: %w", err)
			}
			if err := groupInto(
				`SELECT item_type, count(*) FROM entries WHERE `+cond+` GROUP BY item_type`,
				out.ByItemType); err != nil {
				return fmt.Errorf("store: dashboard item type: %w", err)
			}
			rows, err := tx.Query(ctx, `
				SELECT coalesce(platform_name, ''), count(*) FROM entries
				WHERE `+cond+` GROUP BY 1 ORDER BY count(*) DESC, 1`, args...)
			if err != nil {
				return fmt.Errorf("store: dashboard platforms: %w", err)
			}
			byPlatform, err := pgkit.ScanAll(rows, []PlatformCount{}, func(r pgx.Rows) (PlatformCount, error) {
				var p PlatformCount
			...
			if err != nil {
				return fmt.Errorf("store: dashboard spend: %w", err)
			}
			spend, err := pgkit.ScanAll(srows, []CurrencySpend{}, func(r pgx.Rows) (CurrencySpend, error) {
				var c CurrencySpend
	...
	if txErr != nil {
		return DashboardCounts{}, txErr
	}
	return out, nil
}

// PricingRow is the slim projection the dashboard's value composition needs
// per entry (including disabled, counted as excluded); ProductID is nil for customs.
...
func (s *Store) PricingRows(ctx context.Context, userID uuid.UUID, f Filters) ([]PricingRow, error) {
	...
	return pgkit.ScanAll(rows, []PricingRow{}, func(r pgx.Rows) (PricingRow, error) {
		var row PricingRow
