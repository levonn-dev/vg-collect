...
type ServerInterface interface {
	...
	// Comment on a shelf (validates the shelf via collection)
	// (POST /shelves/{shelfId}/comments)
	CreateShelfComment(w http.ResponseWriter, r *http.Request, shelfId openapi_types.UUID)
	// Account-deletion leg - purge the caller's social graph (idempotent)
	// (DELETE /user-data)
...
func HandlerWithOptions(si ServerInterface, options StdHTTPServerOptions) http.Handler {
	...
	m.HandleFunc("GET "+options.BaseURL+"/comments/by-ids", wrapper.GetCommentsByIds)
	m.HandleFunc("DELETE "+options.BaseURL+"/comments/{commentId}", wrapper.DeleteComment)
	m.HandleFunc("POST "+options.BaseURL+"/events/shelf-published", wrapper.RecordShelfPublished)
	m.HandleFunc("GET "+options.BaseURL+"/explore/top-shelves", wrapper.GetTopShelves)
	m.HandleFunc("GET "+options.BaseURL+"/feed", wrapper.GetFeed)
	m.HandleFunc("DELETE "+options.BaseURL+"/follows/{userId}", wrapper.Unfollow)
	m.HandleFunc("PUT "+options.BaseURL+"/follows/{userId}", wrapper.Follow)
	m.HandleFunc("DELETE "+options.BaseURL+"/likes/{shelfId}", wrapper.UnlikeShelf)
	m.HandleFunc("PUT "+options.BaseURL+"/likes/{shelfId}", wrapper.LikeShelf)
	m.HandleFunc("GET "+options.BaseURL+"/profiles/{userId}/summary", wrapper.GetProfileSocialSummary)
	m.HandleFunc("GET "+options.BaseURL+"/shelves/summary", wrapper.GetShelvesSocialSummary)
	m.HandleFunc("GET "+options.BaseURL+"/shelves/{shelfId}/comments", wrapper.ListShelfComments)
	m.HandleFunc("POST "+options.BaseURL+"/shelves/{shelfId}/comments", wrapper.CreateShelfComment)
