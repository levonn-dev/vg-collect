...
graph LR
    ...
    eso[ExternalSecret auth-secrets] -.->|"signing seed, provider credentials, internal service token"| auth
    esopg[ExternalSecret auth-pg-credentials] -.->|"pg password"| auth
...
values fill most of them; secrets arrive through the ExternalSecrets
`auth-secrets` and `auth-pg-credentials` (both ClusterSecretStore
`vg-fake` in dev, refreshed every 1m), whose store keys Tilt renders
from `.env` (see `.env.example`).

| Env var                                     | Default                                       | Comes from                                                                                                                                                                                                                                                      |
| ------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `HTTP_ADDR`                                 | `:8080`                                       | code default; the chart does not set it                                                                                                                                                                                                                         |
| `DATABASE_URL`                              | required                                      | assembled in deployment.yaml: `postgres://auth:$(PG_PASSWORD)@auth-pg:5432/auth?sslmode=verify-full&sslrootcert=/etc/vg/pg-ca/ca.crt`; `PG_PASSWORD` from Secret `auth-pg-credentials` key `password` (store key `auth/pg-password`, `.env` `PG_AUTH_PASSWORD`) |
