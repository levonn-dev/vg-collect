...
export default function PromoteCandidates() {
  ...
  if (list.isPending) return <p className="mt-4 text-sm text-gray-500">Loading candidates...</p>
  if (list.isError)
    return (
      <p role="alert" className="mt-4 text-sm text-red-700">
        The promote candidates could not be loaded.
      </p>
    )
  ...
  return (
    <section aria-label="Promote candidates" className="mt-6">
      <h3 className="text-base font-semibold">
        {total} community {total === 1 ? 'product' : 'products'} with possible provider matches
      </h3>
      <table className="mt-2 w-full text-left text-sm">
        <thead>
          <tr className="border-b border-gray-200 text-gray-500">
            ...
            <th className="py-1 pr-2 font-normal">Top candidate</th>
            <th className="py-1 pr-2 font-normal">Last confirmed</th>
